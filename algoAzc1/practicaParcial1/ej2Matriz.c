...
void suma_elementos_de_fila(TvecFilas matrizEjemplo ,int *vector_resultado){
	int i , j;
	for(i=0; i<cantfilas; i++){
		vector_resultado[i]=0;
		for (j=0; j<cantcols; j++){
			vector_resultado[i]+=matrizEjemplo[i][j];
		}
	}
}

int main(){
	int vector_resultado[cantfilas];
	TvecFilas matrizEjemplo = {
	...
	suma_elementos_de_fila(matrizEjemplo, vector_resultado);
	for (int i = 0; i<cantfilas; i++){
		printf("la suma de la fila da: %d - ", vector_resultado[i]);
		printf("posicion del vector numero: %d\n", i);
	}
